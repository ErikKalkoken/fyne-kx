...
import (
	...
	"github.com/ErikKalkoken/fyne-kx/internal/syncqueue"
)

// waitTimeout is how long a test waits for a goroutine result before
// declaring the test failed, instead of hanging forever.
const waitTimeout = 2 * time.Second

// settleDelay gives a goroutine enough real time to reach a blocking wait
// before the test proceeds to cancel it or hand it an item.
...
func TestSyncQueue_Get_MultipleWaiters(t *testing.T) {
	t.Run("should wake up the correct waiter and leave others waiting", func(t *testing.T) {
		...
		case <-time.After(waitTimeout):
			t.Fatal("timed out waiting for waiter 1 to be canceled")
		}

		// Waiter 2 should still be waiting until an item is pushed
		select {
		case <-resCh2:
			t.Fatal("waiter 2 returned before an item was available")
		default:
		}
		...
		case <-time.After(waitTimeout):
			t.Fatal("timed out waiting for waiter 2 to return")
		}
	})
	...
	t.Run("should fulfill multiple waiters sequentially as items are added", func(t *testing.T) {
		...
		for range 2 {
			...
			case v := <-results:
				got = append(got, v)
			case <-time.After(waitTimeout):
				t.Fatal("timed out waiting for a waiter to return")
			}
		}
